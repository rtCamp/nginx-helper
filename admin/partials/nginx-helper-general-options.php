											...
											<?php
												echo wp_kses(
													sprintf(
														...
														esc_html__( 'Uses the', 'nginx-helper' ),
														esc_html__( 'module', 'nginx-helper' )
													),
													array(
														'strong' => array(),
														'a' => array( 'href' => array() ),
											...
											<?php
												echo wp_kses(
													sprintf(
														...
														esc_html__( 'Checks for matching cache file in ', 'nginx-helper' ),
														esc_html__( 'Does not require any other modules. Requires that the cache be stored on the same server as WordPress. You must also be using the default nginx cache options (levels=1:2) and (fastcgi_cache_key "$scheme$request_method$host$request_uri").', 'nginx-helper' )
													),
													array( 'strong' => array() )
												);
											?>
							...
							<?php
								echo wp_kses(
									sprintf(
										...
										esc_html__( 'Can\'t write on map file.', 'nginx-helper' ),
