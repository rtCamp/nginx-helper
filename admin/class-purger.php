...
abstract class Purger
{
	...
	{
		add_action('nginx_helper_unlink_recursive', [$this, 'run_cron_unlink_recursive'], 10, 2);
	}
	...
	public function unlink_recursive($dir, $delete_root_too)
	{
		...
		if (!is_dir($dir)) {
			$this->log("{$dir} is not available. Stopping.");
			return;
		}
		...
		if (!$dh) {
			$this->log("{$dir} resource not available. Stopping.");
			return;
		}
	...
	public function run_cron_unlink_recursive($dir, $delete_root_too)
	{
		$this->log('* * * * *');
		$this->log('* Starting to purge everything..');

		$this->unlink_recursive($dir, $delete_root_too);

		$this->log('* Purged Everything done.');
		$this->log('* * * * *');
	}
}
